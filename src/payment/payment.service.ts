...
export class PaymentService {
  ...
  async activateFreeTrial(token: string) {
    ...
    if (!status) {
      return {
        status: false,
        message:
          "free trial don't start because the information are not complete!!!",
      };
    }
    if (
  ...
  async checkFreeTrialUsage(token: string) {
    const { accountType, startPlan, endPlan, status } =
      await this.businessService.checkBusinessForActivation(token, false);
